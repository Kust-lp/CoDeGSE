...
def NCGame(adj_list_name,gt_comm,name, out_path, stopCRITERION=0.01,similarity_type='HP'):
    ...
    print(f"\ntime consuming:{time_diff}")

    with open(out_path+'NCGame.txt' , 'w', encoding='utf8') as f:
        for c in pre_comm:
...
if __name__ == '__main__':
    for name in [ "amazon" ,"youtube", "dblp","LiveJournal","orkut", "friendster","wiki"]:
        ...
        NCGame(DATASETS[name]["graph"],DATASETS[name]["ground_truth"],name,out_path)